...
auto objective = make_shared<ObjectiveIK>();
auto optimizerGDLS = make_shared<OptimizerGDLS>(NLINKS);
auto optimizerNM = make_shared<OptimizerNM>(NLINKS);

void wrapAngle(VectorXd& angle) {
	for (int i = 0; i < angle.rows(); i++) {
		while (angle(i, 0) > glm::pi<double>()) {
			angle(i) -= 2 * glm::pi<double>();
		}
		while (angle(i, 0) < -glm::pi<double>()) {
			angle(i) += 2 * glm::pi<double>();
		}
...
static void cursor_position_callback(GLFWwindow* window, double xmouse, double ymouse)
{
	...
	if(keyToggles[(unsigned)' ']) {
		optimizerGDLS->toggleDebug(keyToggles[(unsigned)'d']);
		optimizerNM->toggleDebug(keyToggles[(unsigned)'d']);
		objective->setTarget(x);
		objective->setLink(links[0]);
		VectorXd angleInit(NLINKS);
		for (int i = 0; i < NLINKS; i++) {
			angleInit(i) = links[i]->getAngle();
		} 
		VectorXd angleGDLS(NLINKS);
		VectorXd angleNM(NLINKS);
		VectorXd angle(NLINKS);
		angleGDLS = optimizerGDLS->optimize(objective, angleInit);
		wrapAngle(angleGDLS);
		double fAngleGDLS = objective->evalObjective(angleGDLS);
		angleNM = optimizerNM->optimize(objective, angleGDLS);
		wrapAngle(angleNM);
		double fAngleNM = objective->evalObjective(angleNM);
		angle = fAngleNM < fAngleGDLS ? angleNM : angleGDLS;
		for (int i = 0; i < NLINKS; i++) {
			links[i]->setAngle(angle(i));
		}
		cout << optimizerGDLS->getIter() << " GDLS iterations" << endl;
		cout << optimizerNM->getIter() << " NM iterations" << endl;
	}
}
...
static void init()
{
	...
	for (int i = 0; i < NLINKS; i++) {
		...
		links.push_back(l);
		if (i != 0) {
			links[i - 1]->addChild(l);
		}
	}
	objective->setNLink(NLINKS);
}
